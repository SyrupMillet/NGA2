...
!           IA and JA allocatable arrays. Refer to the documentation
...
!     L*****     - lengths and allocatables for some internal arrays
...
!         RELEASE_OPTS_ARRAYS can be called to release/deallocate the opts arrays 
          MODULE PROCEDURE RELEASE_OPTS_ARRAYS
...
!         DVDSM can be called to calculate sparse allocatable arrays needed
!         by JACSP
...
! .. Derived Type Declarations ..
      TYPE, PUBLIC :: VODE_OPTS
        REAL (WP), DIMENSION (:), POINTER :: ATOL=>NULL()
        REAL (WP), DIMENSION (:), POINTER :: RTOL=>NULL()
...
! structure allocatablee array that are determined internally
...
!    (3) you wish to supply the sparse allocatable arrays
!        directly,
...
!            define the sparse structure array allocatables directly
!
! NEVENTS
!
! Default:   0
...
! structure allocatable arrays IA and JA directly for DVODE_F90
! (not called by DVODE_F90).
! ..
...
! ..
!     Approximate or allow the user to supply the sparse Jacobian
!     structure allocatable arrays IA and JA for DVODE_F90 (not called
!     by DVODE_F90).
! ..
!     Caution:
!     If it is called, SET_IAJA must be called after the call to
...
! LOCJS  = A allocatable to the saved Jacobian, whose storage starts at
!          WM(LOCJS), if JSV = 1.
! LYH    = Saved integer allocatable to segments of RWORK and IWORK.
...
! Set work array allocatables and check lengths LRW and LIW. Pointers
...
!              groups
!     IGP    = allocatable array of length NGRP + 1 to the locations
...
!       BIGP    = integer allocatable array of length BNGRP + 1
...
!       recomputed. If MA28 = 1,2 the JVECT and ICN allocatable arrays
!       will be defined and MA28AD will be called to decompose
!       PMAT. If MA28 = 3 the JVECT allocatable array will be defined
!       and MA28BD will be called to decompose PMAT using the ICN
!       allocatable array returned in the last call to MA28AD.
...
!           Reuse the saved Jacobian.
          ...
          IF (MA28==1 .OR. MA28==2) THEN
            ...
            END IF
!           Define column allocatables for MA28AD.
            CALL SET_ICN(N,IAN,ICN)
            CALL CHECK_DIAG(N,IAN,JAN,ICN)
...
!           Reuse the saved constant Jacobian.
          ...
          ELSE
            ...
            IF (.NOT.(J_IS_CONSTANT.AND.J_HAS_BEEN_COMPUTED)) THEN
               IF (USE_JACSP) THEN
!                 Approximate the Jacobian using Doug Salane's JACSP.
!                 The JPNTRDS and INDROWDS allocatable arrays were defined
                  ...
                  DO NG = 1, MAXGRPDS
                    ...
                    DO JJ = JJ1, JJ2
                      ...
                      DO K = K1, K2
                        I = JAN(K)
                        GOTO (17,17,18) MA28
!                       Define the row allocatables for MA28AD.
17                      JVECT(K) = I
!                       Define the column allocatables for MA28AD.
                        ICN(K) = J
                        GOTO 19
!                       Define the column allocatables for MA28AD.
18                      JVECT(K) = J
                  ...
                  DO NG = 1, NGP
                    ...
                    DO JJ = JJ1, JJ2
                      ...
                      DO K = K1, K2
                        I = JAN(K)
                        GOTO (20,20,30) MA28
!                       Define row allocatables for MA28AD.
20                      JVECT(K) = I
!                       Define column allocatables for MA28AD.
                        ICN(K) = J
                        GOTO 40
...
!_______________________________________________________________________
      ...
      SUBROUTINE RELEASE_ARRAYS(QUIET)
! ..
...
!       In case DVODE_F90 is subsequently called:
      ...
      SUBROUTINE RELEASE_OPTS_ARRAYS(OPTS)
! ..
! Deallocate any allocated arrays in the opts type 
     IMPLICIT NONE
! ..
! .. Scalar Arguments ..
        TYPE (VODE_OPTS), INTENT(INOUT) :: OPTS
! ..
! .. Local Scalars ..
        INTEGER :: IER
        CHARACTER (80) :: MSG
...
! ..
! .. FIRST EXECUTABLE STATEMENT RELEASE_OPTS_ARRAYS
! ..
        IF (ASSOCIATED(OPTS%RTOL)) THEN
          DEALLOCATE (OPTS%RTOL,STAT=IER)
          NULLIFY (OPTS%RTOL)
          CALL CHECK_STAT(IER,880)
        END IF
        IF (ASSOCIATED(OPTS%ATOL)) THEN
          DEALLOCATE (OPTS%ATOL,STAT=IER)
          NULLIFY (OPTS%ATOL)
          CALL CHECK_STAT(IER,880)
        END IF
...
!       In case DVODE_F90 is subsequently called:
      ...
      END SUBROUTINE RELEASE_OPTS_ARRAYS
...
!       Count the number of diagonal blocks and set up allocatables to the
...
120       DO II = IACTIV, ITOP
            ...
            LENC(I) = LENC(I) + 1
          END DO
!         Set up column allocatables so that IPC(J) points to position
...
!       allocatable. This is so that the beginning of each row/col can
...
!       Reset first entry of each row/col and allocatable array IPTR.
        DO 30 K = 1, KL
          JPOS = ITOP - K + 1
...
! ..
        ...
        IFLAG = 0
        W(1:N) = ZERO
!       Set up allocatables to the beginning of the rows.
        IW(1) = IDISP(1)
        IF (N==1) GOTO 30
        DO I = 2, N
...
!     Set up allocatables IW(:,1) to the beginning of the rows
...
!       with these row allocatable and length arrays has maximum
...
90          END DO
            ...
            IBEG = LICN + 1
!           Reset allocatables to the beginning of the rows.
            DO 100 I = 2, N
...
!                indices by groups
!     IGP      = allocatable array of length NGRP + 1 to the
...
! JPNTR and INDROW sparse allocatable arrays in the event a dense or a
